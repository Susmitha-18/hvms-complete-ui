...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        // Skip map fetch if disabled
        if (typeof window !== 'undefined' && window.__HVMS_SKIP_MAP_FETCH) {
          console.log('🚫 Map fetch skipped (production mode)');
          setVehicles([]);
          return;
        }
        ...
        console.log(`📍 Processing ${list.length} vehicles...`);
        
        const out = [];
        for (const v of list) {
          try {
            if (!v.address || !v.address.trim()) {
              console.log(`⚠️ Skipping vehicle ${v.id}: no address`);
              continue;
            }

            const g = await geocode(v.address);
            if (!g) {
              console.log(`⚠️ Skipping vehicle ${v.id}: geocoding failed`);
              continue;
            }
            ...
            console.error(`❌ Error processing vehicle:`, v.id, vErr);
          }
        }

        console.log(`✅ Processed ${out.length} vehicles successfully`);
        setVehicles(out);
      } catch (err) {
