...
// ✅ Generate PDF Report (Fixed Fonts + Layout)
const handleDownload = () => {
  try {
    ...
    alert("⚠️ Failed to generate report. Please check console.");
  }
};
    ...
    return (
        <div className="max-w-6xl mx-auto p-8 bg-white rounded-xl shadow-lg mt-6 mb-12">
            ...
            {/* Maintenance History */}
            <section>
                <h2 className="text-xl font-semibold text-gray-700 mb-4">🔧 Maintenance History</h2>
                {records.length === 0 ? (
                            ...
                            <tbody>
                                {records.map((r, i) => (
                                    <tr key={r._id} className="border-t hover:bg-gray-50">
                                        ...
                                        <td className="p-3 border">{r.serviceType || "—"}</td>
                                        <td className="p-3 border">{r.replacedParts || "—"}</td>
                                        <td className="p-3 border text-right">₹{r.totalCost || 0}</td>
                                        <td className="p-3 border">{r.serviceDate?.split("T")[0] || "—"}</td>
                                        <td className="p-3 border">{r.startDate?.split("T")[0] || "—"}</td>
                                        <td className="p-3 border">{r.endDate?.split("T")[0] || "—"}</td>
                                    </tr>
                                ))}
                                <tr className="bg-gray-50 font-semibold text-gray-800">
                                    ...
                                    <td className="p-3 border text-right">₹{totalCost}</td>
                                    <td colSpan={3}></td>
                                </tr>
                            </tbody>
                        </table>
