...
export default function VehicleHistory() {
  ...
  const handleSave = async (recordId, index) => {
    ...
    try {
      // Use apiFetch so runtime API overrides (window.__HVMS_API_URL) are respected
      const res = await apiFetch(`/api/vehicles/history/update/${recordId}`, {
        method: "PUT",
      ...
      if (res.ok) {
        // Update local state with the returned updated document (if provided)
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 bg-white shadow rounded-xl mt-6 mb-12">
      ...
      {/* Add map to show current/last location */}
      <div className="mb-8">
        <h2 className="text-xl font-semibold text-gray-700 mb-2">Current Location</h2>
        <MapCard 
          vehicleLocations={history.length > 0 ? [{
            id: vehicle._id,
            vehicleId: vehicle.vehicleId,
            vehicleName: vehicle.name,
            registrationNumber: vehicle.registrationNumber,
            model: vehicle.vehicleModel,
            address: history[0].clientAddress,  // Use most recent address
            status: vehicle.status
          }] : []}
          lastUpdatedText={history.length > 0 ? new Date(history[0].assignedAt).toLocaleString() : 'No history'}
        />
      </div>
